...
(function(global, $) {
  ...
  $.fn.Paginacao.methods = {
    ...
    __showLoading__: function() {
      _self.domCache["$buttonPaginacao"].html(
    ...
    __montarNoticia__: function(noticia) {
      return (
        '<li class="materia" itemscope itemtype="https://schema.org/NewsArticle">' +
        '	<div class="materia-chamada">' +
        '		<span class="materia-data" itemprop="publishDate" content="' +
        noticia.ultima_atualizacao +
        '">' +
        noticia.ultima_atualizacao +
        "</span>" +
    ...
    __controleBotoesPaginacao__: function() {
      var totalPaginas = Math.ceil(
        parseInt(_self.options.total, 10) / _self.options.itensPorPagina
      );
    ...
    __carrega_noticias__: function() {
      ...
      var urlRequest =
        this.options.urlJSON + (_self.options.paginaAtual + 1) + ".json";
    ...
    __bindEvents__: function() {
      _self.domCache["$buttonPaginacao"].die("click").live("click", function() {
        if (
          _self.options.total >
          _self.options.paginaAtual * _self.options.itensPorPagina
        ) {
